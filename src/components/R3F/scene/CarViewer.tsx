...
import type { Catalog, VisualEffect } from '../../../types/config.ts'
import { selectedOptionIds } from '../../../lib/rules.ts'
...
export const CarViewer = () => {
  ...
  const { root, materials, baseline } = useMemo(() => {
    ...
    return { root, materials, baseline }
  }, [scene])

  // Our clones are ours to dispose; the cached originals are not.
  useLayoutEffect(() => {
    return () => {
      for (const list of materials.values()) for (const m of list) m.dispose()
    }
  }, [materials])
  ...
  }, [root, materials, baseline, catalog, build])
